...
# in file sample.list.txt (update #SBATCH --array=1-501:1)
#######################################################################################################
...
do
  ...
  mv fastp.html /data/biol-gt-genomics/sjoh4959/wm-gwas/reports/raw_read_QC/${ReadPair}.html
  mv fastp.json /data/biol-gt-genomics/sjoh4959/wm-gwas/reports/raw_read_QC/${ReadPair}.json
...
REF=/data/biol-wm/sjoh4959/0.0_winter-moth/data/ref_genome/GCA_932527175.1/GCA_932527175.1_ilOpeBrum1.1_genomic_renamed.
fna
...
for ReadPair in `ls Filtered_${SAMPLE_NAME}_*_1.fq.gz | cut -f1,2,3,4,5,6 -d'_'`
do
  	$BWA mem $REF \
  	${ReadPair}_1.fq.gz \
  	${ReadPair}_2.fq.gz	\
